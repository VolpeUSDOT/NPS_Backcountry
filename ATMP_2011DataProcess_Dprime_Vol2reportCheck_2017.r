...
#######################################################################
#Filter d prime data to limit range (this removes ~1000 rows, mostly due to NA and many of these are removed in step to generate complete observations below)
#We should include only DprimeLSELAllAC 0-80 and both DprimeLLeqTAC and DprimeLeqTresp 0-50.
dim(DRMerged2011sub)
write.csv(DRMerged2011sub, file="F:/Volpe/Rwork/DprimeScripts/DRMerged2011subset_dprime_2017checkv1a.csv")
hist(DRMerged2011sub$DprimeLSELAllAC)
length(which(DRMerged2011sub$DprimeLSELAllAC < 80))
length(which(is.na(DRMerged2011sub$DprimeLSELAllAC))==TRUE)
DRMerged2011subLim <- subset(DRMerged2011sub,DRMerged2011sub$DprimeLSELAllAC <= 80)
DRMerged2011subLim <- subset(DRMerged2011sub,DRMerged2011sub$DprimeLLeqTAC <= 50)
length(which(DRMerged2011sub$DprimeLLeqTAC < 50))
length(which(is.na(DRMerged2011sub$DprimeLLeqTAC))==TRUE)
hist(DRMerged2011sub$DprimeLLeqTAC)
DRMerged2011subLim <- subset(DRMerged2011sub, DprimeLeqTresp <= 50)
dim(DRMerged2011subLim)

#######################################################################

#Change names in dataset to match code for model fitting
names(DRMerged2011subLim)[names(DRMerged2011subLim)=="site"] <- "Site"
Dataset <- rep("Prior",nrow(DRMerged2011subLim))
SeqAll <- c(1:nrow(DRMerged2011subLim))

DRMerged2011subLim <- data.frame(DRMerged2011subLim,Dataset,SeqAll)
names(DRMerged2011subLim)

DRMerged2011subLim <- subset(DRMerged2011subLim,DRMerged2011sub$Survey!="")
DRMerged2011subLim <- droplevels(DRMerged2011subLim)
levels(DRMerged2011subLim$Survey)

#Save full datafile to use for model fitting
write.csv(DRMerged2011subLim, file="F:/Volpe/Rwork/DprimeScripts/DRMerged2011subset_dprime_2017check.csv")
names(DRMerged2011subLim)
dim(DRMerged2011subLim)
str(DRMerged2011subLim)
...
dose.cols <- DRMerged2011subLim[,complete.dose]
dose.rows <- complete.cases(dose.cols)
length(dose.rows)

DRMerged2011subComplete <- DRMerged2011subLim[dose.rows==TRUE,]
...
write.csv(DRMerged2011subComplete, file="F:/Volpe/Rwork/DprimeScripts/ATMP2011_CompleteDoseVars_dprime.csv")
