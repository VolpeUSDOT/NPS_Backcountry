...
### Produces dose response curves for the Annoy responses.
...
varnames.plot = c(DesiredPlotY, vars.dos, vars.interact, "SurveyHR1","SurveyHR2", vars.mit[2:length(vars.mit)])
...
#### Adding Yes to vars.testdosmit
vars.dosmit = c(vars.dos, vars.interact, "SurveyHR1","SurveyHR2", vars.mit[2:length(vars.mit)])
...
for (i in 1:numb.lm) {
  ...
  if (vars.dosmit[i] == "SurveyHR1") {          					#Specific var.
    lm.input[vars.dosmit[i], "Constant"] = table(vars.all.data$Survey)[2]/(sum(table(vars.all.data$Survey)))
    lm.input[vars.dosmit[i], "Slope"] = 0
  } 
  if (vars.dosmit[i] == "SurveyHR2") {  									#Specific var.
    lm.input[vars.dosmit[i], "Constant"] = table(vars.all.data$Survey)[3]/(sum(table(vars.all.data$Survey)))
    lm.input[vars.dosmit[i], "Slope"] = 0
  }
...
##########
write.csv(lm.input,file = paste("X:/Overnight/lm_coeff_", DesiredPlotY,"_",paste(vars.dosmit,collapse="_"),"_Annoy.csv",sep=""))
...
if (DesiredRegrType != "Ordinal" & add.plot==FALSE) {	
  ...
  legend(xlimits[1],95,legend=c("Slightly or more","Moderately or more","Very or more"),lty=1,lwd=4, text.font=2,col=c("darkolivegreen", "darkolivegreen3", "honeydew3"),bty="n", xjust=0, y.intersp=.9,cex=1.6)
}
...
##To see random parts
#ranef(fit.plot)$Site
#sum(unlist(ranef(fit.plot)$Site))
