...
for (i in 1:numb.lm) {
  ...
  if (vars.dosmit[i] == "SurveyHR1") {
    if (DataType == "AllCorrected" | DataType == "AllCorrectedOnlyPrior") {
      if (SType == "HR1") {
        lm.input[vars.dosmit[i],"Constant"] = 1
        lm.input[vars.dosmit[i],"Slope"] = 0
      } else {
        lm.input[vars.dosmit[i],"Constant"] = 0
        lm.input[vars.dosmit[i],"Slope"] = 0
      }
    }
  }  
  
  if (vars.dosmit[i] == "SurveyHR2") {
    if (DataType == "AllCorrected" | DataType == "AllCorrectedOnlyPrior") {
      if (SType == "HR2") {
        lm.input[vars.dosmit[i],"Constant"] = 1
        lm.input[vars.dosmit[i],"Slope"] = 0
...
##########
vars.dosmit2 <- sub("\\*", "x", vars.dosmit)

write.csv(lm.input, file = paste("lm_coeff_", DesiredPlotY,"_",paste(vars.dosmit2,collapse="_"),"_Annoy.csv",sep=""))
...
if (DesiredRegrType != "Ordinal" & add.plot==FALSE) {  
  plot(Res.jit ~ vars.all.data[[DesiredPlotX]],
       col=col.line,
       xlim = xlimits,
       main = paste(Title,DesiredPlotY),
       xlab = xlabel, ylab = ylabel, font.lab = 2,
       lab = c(8,10,6), cex = 0.45, cex.lab = 1.8, cex.main = 2, cex.sub = 1.5,
       cex.axis = 1.5
  )
  legend(xlimits[1],95,legend=c("AC","HR1","HR2"),lty=1,lwd=4, text.font=2,col=c("darkolivegreen", "darkolivegreen3", "honeydew3"),bty="n", xjust=0, y.intersp=.9,cex=1.6)
}
...
#####	Plot random parts
	if (RandomParts == TRUE & DesiredRegrType != "Ordinal") {
		library("arm")
		loops = length(ranef(fit.plot))
		for (i in 1:loops) {
			eq.curve = paste(eq.curve.truncParen, " + ", ranef(fit.plot)[i], sep="")
			eq.curve = paste(eq.curve, ")", sep="")
