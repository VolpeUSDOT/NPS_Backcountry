...
##########
write.csv(lm.input,file = paste("lm_coeff_", DesiredPlotY,"_",paste(vars.dosmit,collapse="_"),"_Annoy.csv",sep=""))
...
xnumb = 50
  ...
  xmax = max(vars.all.data[DesiredPlotX])
   xmin = 30 #manually set limits
   xmax = 90
  #xlimits = c(xmin-2, xmax+2)  				#Set x-axis limits
  xlimits = c(30, 90)    			#Set x-axis limits
