...
####  Set up xplot values
xnumb = 50
  xmin = 30 #min(vars.all.data[DesiredPlotX])
  xmax = 90 #max(vars.all.data[DesiredPlotX])
  xlimits = c(xmin-2, xmax+2)    			#Set x-axis limits
  #xlimits = c(25, 115)    			#Set x-axis limits
...
if (DesiredRegrType != "Ordinal" & add.plot==FALSE) {  
  plot(Res.jit ~ vars.all.data[[DesiredPlotX]],
       ...
       lab = c(8,10,6), cex = 0.1, cex.lab = .8, cex.main = 1,
       cex.axis = .8
  )
  legend(xlimits[1],95,legend=c("Slightly or more","Moderately or more","Very or more"),lty=1,lwd=2, text.font=2,col=c("black", "gray40", "gray80"),bty="n", xjust=0, y.intersp=.9,cex=.6)
...
if (EquationCurve == TRUE & DesiredRegrType != "Ordinal") {
	...
	lines(xplot, yplot, col=col.line,lwd=2)
}
...
if (ConfLimits == TRUE & DesiredRegrType != "Ordinal") {
  ...
  eq.conf = gsub("*xplot.comp", "*xplot.comp[x.95]", eq.conf, fixed = T)
	...
	lines(smooth.spline(xplot, y.top, spar=0.6), lwd=1, col = col.line, lty = "dashed")
	lines(smooth.spline(xplot, y.bot, spar=0.6), lwd=1, col = col.line, lty = "dashed")
}
...
if (EquationCurve == TRUE & DesiredRegrType != "Ordinal" & add.plot==FALSE) {
  points(vars.all.data[[DesiredPlotX]], Res.jit, pch=16, cex = 0.1, col="gray40")
